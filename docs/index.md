...
events
Event(0x306f3bdb) Address of the payer, DPlat, Infra and Royalty Fee
...
```solidity
function getRoyaltFeeInZbyte(address dapp_, address user_, bytes4 functionSig_, address payer_, uint256 zbyteCharge_) external view returns (uint256, address, address)
```
...
event (0xf9d1da16): Zbyte DPlat enterprise limit is set.
...
To prepare a contract for DPlat compatibility, users are required to derive from the abstract contract called ZbyteContext.
